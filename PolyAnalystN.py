...
class SolutionFinder(QThread):
    ...
    def find_solutions_for_n(self, n_val):
        try:
            ...
            # Sadece pozitif sayılar için arama yap
            var_ranges = [range(0, self.var_range + 1) for _ in self.variables]
...
class PolyAnalyst(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("PolyAnalyst - Doğal Sayılar")
...
3. n için aralık seçin: [n_başlangıç, n_bitiş] (doğal sayılar)
4. Değişkenler için maksimum değer seçin: [0, max_değer] arası aranır
Not: Bu versiyon sadece doğal sayılar için çözüm arar.
"""
        ...
        self.n_start.setRange(1, 1000000)  # Sadece pozitif
        self.n_start.setValue(1)
        input_group.addWidget(self.n_start, 2, 1)
        
        input_group.addWidget(QLabel("n bitiş:"), 3, 0)
        self.n_end = QSpinBox()
        self.n_end.setRange(1, 1000000)  # Sadece pozitif
        self.n_end.setValue(10)
        input_group.addWidget(self.n_end, 3, 1)
        
        input_group.addWidget(QLabel("Değişken üst sınırı:"), 4, 0)
        self.var_range = QSpinBox()
        self.var_range.setRange(1, 100000)  # Sadece pozitif
        self.var_range.setValue(20)
    ...
    def plot_polynomial(self, expr_str, variables, n_val):
        try:
            if len(variables) >= 2:
                ...
                x = np.linspace(0, self.var_range.value(), 50)  # Sadece pozitif
                y = np.linspace(0, self.var_range.value(), 50)  # Sadece pozitif
                X, Y = np.meshgrid(x, y)
                ...
                x = np.linspace(0, self.var_range.value(), 200)  # Sadece pozitif
                expr = sp.sympify(expr_str)
    ...
    def start_analysis(self):
        try:
            ...
            if self.n_end.value() < self.n_start.value():
                raise ValueError("Bitiş değeri başlangıç değerinden küçük olamaz")
    ...
    def show_all_results(self):
        ...
        text.append(f"Değişken aralığı: [0, {self.var_range.value()}]")
        text.append("\nSonuçlar:")
        text.append("-" * 50)
